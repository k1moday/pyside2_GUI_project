...
class Graph:
    ...
    def adjacency_matrix(self):
        ...
        col = np.array(
            [nodeIndexDict[src_key] for src_key, valdict in self.edges.items() for trgt_key in valdict.keys()])
        row = np.array(
            [nodeIndexDict[trgt_key] for src_key, valdict in self.edges.items() for trgt_key in valdict.keys()])
