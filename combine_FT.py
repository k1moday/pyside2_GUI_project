...
class FTToimportance:

    def monteCarlo(self, lam_, strsChanged_, baseEvent_, x_, trial_, n_):
    ...
    def getBaseEvents(self, matrix):
        baseEvents = []
        for index in range(len(matrix)):
            if matrix[index][2] == 0:
                baseEvents.append(matrix[index][0])
        return baseEvents

    def getImportance(self, matrix, trial, lam, baseEvent):
        ...
        # baseEvent = self.getBaseEvents(matrix)
        # baseEvent = [3, 7, 8, 6, 4]

        x = 100
        topPro1 = self.monteCarlo(lam, strsChanged, baseEvent, x, trial, mOriginal)
        ...
        topPro2 = self.monteCarlo(lam, strsChanged, baseEvent, x, trial, mOriginal)

        nodeImportance = [0.0 for _ in range(len(baseEvent))]
        for i in range(len(baseEvent)):
